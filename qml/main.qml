...
Window {
  ...
  // ───────── Unten über volle Breite: NEWS-TICKER (fix) ─────────
  Rectangle {
    id: newsBar
    anchors.left: parent.left
    anchors.right: parent.right
    ...
    // Etwas höher + dynamisch: genug Luft für Unterlängen (g, p, ä …)
    ...
    // dezenter Hintergrund für Kontrast
    Rectangle { anchors.fill: parent; color: "#000"; opacity: 0.18 }

    // Clip-Bereich: seitlich 24, oben/unten nur vpad
    Item {
      id: clipper
      anchors.left: parent.left
      anchors.right: parent.right
      ...
      Text {
        id: newsText
        text: "News …"
        // immer sauber mittig, kein manuelles y-Rechnen
        anchors.verticalCenter: parent.verticalCenter
        x: 0
        font.pixelSize: newsBar.fontPx
        font.family: "DejaVu Sans"
        // NativeRendering = scharf & korrektes Glyph-Boxing
    ...
    // sanfter Wechsel: Fade-Out → Text tauschen → Messen → (evtl.) Scroll
  ...
  Connections {
    ...
    function onCpuChanged(s)       { cpuTxt.text = s }
